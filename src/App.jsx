...
export default function App() {
  ...
  const executeSearch = async (type, query) => {
    try {
      const response = await fetch(SCRIPT_URL, {
        ...
        body: JSON.stringify({ action: 'searchInvoice', password: appPassword, type: type, query: query })
      });
      const data = await response.json();
  ...
  const handleInvoiceDropdownSelect = (e) => {
    const val = e.target.value;
    if (val === 'NEW') { setIsInvoiceDropdown(false); handleClearSearch(); setDate(date); } 
    else if (val) { const selected = availableInvoices.find(inv => inv.invoiceNo === val); if (selected) populateDashboard(selected); }
  };
  ...
  const handleInputChange = (id, field, value) => {
    const newItems = items.map(item => {
      if (item.id === id) {
        ...
        const qtyNum = parseFloat(updatedItem.qty) || 0; const priceNum = parseFloat(updatedItem.price) || 0; const discountNum = parseFloat(updatedItem.discount) || 0;
  ...
  const submitInvoiceData = async () => {
    ...
    } catch (error) { alert("Connection error."); return false; }
  };

  const handleSaveOnly = async () => { const isSaved = await submitInvoiceData(); if (isSaved) { alert(`Success! Invoice saved.`); resetDashboardAfterSave(); } };
  
  const generatePDFDocument = async () => {
    const wrapper = printRef.current; if (!wrapper) return false;
    const pageElements = wrapper.querySelectorAll('.print-page'); if (pageElements.length === 0) return false;
    try {
      ...
      for (let i = 0; i < pageElements.length; i++) {
        ...
        if (i > 0) pdf.addPage(); pdf.addImage(imgData, 'JPEG', 0, 0, pdfWidth, pdfHeight, undefined, 'FAST');
      }
      pdf.save(`UrbanEggs_Invoice_${invoiceNo}.pdf`); return true;
    } catch (error) { alert("PDF generation failed."); return false; }
  };
  ...
  if (validItemsToPrint.length <= PAGE_1_MAX) { paginatedItems.push(validItemsToPrint); } 
  else {
    paginatedItems.push(validItemsToPrint.slice(0, PAGE_1_MAX)); let remainingItems = validItemsToPrint.slice(PAGE_1_MAX);
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'flex', gap: '5px' }}>
            {isInvoiceDropdown ? (
              ...
              <>
                <input type="text" className="smart-field" value={invoiceNo} onChange={(e) => setInvoiceNo(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleInvoiceSearch()} disabled={isViewingPast && !isUnlocked} style={{ flex: 1 }} />
                <button onClick={handleInvoiceSearch} disabled={isViewingPast && !isUnlocked} title="Search by Invoice Number" style={{ padding: '0 12px', border: '1px solid #ffcc00', borderRadius: '4px', background: '#fff9e6', cursor: 'pointer', fontSize: '18px' }}>🔍</button>
              </>
            )}
          </div>
        </div>
        ...
        <div className="form-group">
          <label>
            ...
            {client && client !== 'NEW' && (!isViewingPast || isUnlocked) && (
              <span className="edit-client-toggle" onClick={() => { const cData = clientsDb.find(c => c.name === client); setEditClientData({ name: cData.name, phone: cData.phone, address: cData.address, notes: cData.notes }); setShowEditClientModal(true); }}>[✎ Edit]</span>
            )}
            {client && (
          ...
          {showAddFunds && !isViewingPast && (
            <div className="add-funds-popover">
              ...
              <select value={advancePayMode} onChange={(e) => setAdvancePayMode(e.target.value)} className="payment-mode-select" style={{ marginLeft: '10px', padding: '5px' }}>
                <option value="Cash">Cash</option><option value="UPI">UPI</option><option value="Bank Transfer">Bank Transfer</option><option value="Cheque">Cheque</option><option value="Apply Advance" style={{color: '#2e7d32', fontWeight: 'bold'}}>Apply Advance</option>
              </select>
      ...
      <div className="inventory-container" style={{ pointerEvents: (isViewingPast && !isUnlocked) ? 'none' : 'auto', opacity: (isViewingPast && !isUnlocked) ? 0.7 : 1 }}>
        <div className="inventory-list">
          <div className="inventory-header">
            <div>Description</div><div>Qty</div><div>Unit</div><div>Total Count</div><div>Unit Cost (₹)</div><div>Discount (₹)</div><div>Subtotal</div><div></div>
          </div>
          {items.map((item) => (
            <div key={item.id} className="inventory-row">
              <div data-label="Description"><input type="text" className="smart-field" value={item.desc} onChange={(e) => handleInputChange(item.id, 'desc', e.target.value)} placeholder="Item description" /></div>
              <div data-label="Qty"><input type="text" className="smart-field" inputMode="decimal" value={item.qty === 0 ? '' : item.qty} onFocus={(e) => e.target.select()} onChange={(e) => handleInputChange(item.id, 'qty', e.target.value)} placeholder="0" /></div>
              <div data-label="Unit">
                <select className="smart-field" style={{ padding: '8px' }} value={item.unit} onChange={(e) => handleInputChange(item.id, 'unit', e.target.value)}><option value="Trays">Trays</option><option value="Birds">Birds</option><option value="Loads">Load</option><option value="Other">Other</option></select>
              </div>
              <div data-label="Total Count" style={{ display: 'flex', alignItems: 'center', paddingLeft: '5px' }}>{item.totalCount}</div>
              <div data-label="Unit Cost (₹)"><input type="text" className="smart-field" inputMode="decimal" value={item.price === 0 ? '' : item.price} onFocus={(e) => e.target.select()} onChange={(e) => handleInputChange(item.id, 'price', e.target.value)} placeholder="0.00" /></div>
              <div data-label="Discount (₹)"><input type="text" className="smart-field" inputMode="decimal" value={item.discount === 0 ? '' : item.discount} onFocus={(e) => e.target.select()} onChange={(e) => handleInputChange(item.id, 'discount', e.target.value)} placeholder="0.00" /></div>
              <div data-label="Subtotal" className="inv-subtotal-wrapper" style={{ display: 'flex', alignItems: 'center' }}><span className="inv-subtotal">₹{formatINR(item.subtotal)}</span></div>
              <div className="remove-btn-wrapper" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}><span className="btn-remove-modern" title="Remove Item" onClick={() => removeRow(item.id)}>×</span></div>
            </div>
          ))}
        </div>
        ...
        {isViewingPast && !isUnlocked ? (
          historicalLedger?.status === 'Paid' ? (
            <>
              ...
              <span style={{ color: '#2e7d32', fontWeight: 'bold', padding: '12px 24px', background: '#e8f5e9', border: '1px solid #c8e6c9', borderRadius: '6px', display: 'flex', alignItems: 'center' }}>🔒 Invoice Paid & Closed</span>
              <button className="btn-generate-pdf" onClick={handleReprintOnly}><span style={{ filter: 'grayscale(100%)' }}>🖨️</span> Re-Print PDF</button>
            </>
          ) : (
            <>
              <button className="btn-secondary" onClick={handleClearSearch} style={{ marginRight: 'auto' }}>❌ Clear Search</button>
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ minWidth: '350px' }}>
            ...
            <div className="modal-buttons">
              ...
              <button className="btn-modal-cancel" onClick={() => { setShowAddClientModal(false); setNewClientData({name:'', phone:'', address:'', notes:''}); setClient(''); }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div key={pageIndex} className="print-page" style={{ position: 'relative', width: '210mm', height: '297mm', backgroundColor: 'white', overflow: 'hidden', pageBreakAfter: 'always' }}>
            ...
            <div className="print-content">
              ...
              <table className="print-table">
                <thead><tr><th>Description</th><th>Qty</th><th>Total Count</th><th>Rate</th><th>Discount</th><th>Amount</th></tr></thead>
                <tbody>
                  {pageChunk.map(item => (
                    <tr key={item.id}><td>{item.desc || 'Item'}</td><td>{formatCount(item.qty)} {item.unit}</td><td>{formatCount(item.totalCount)}</td><td>₹{formatINR(item.price)}</td><td>₹{formatINR(item.discount)}</td><td>₹{formatINR(item.subtotal)}</td></tr>
                  ))}
                </tbody>
              </table>
